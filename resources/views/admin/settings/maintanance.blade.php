        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-primary card-outline">
                    ...
                    <!-- /.box-header -->
                    <div class="card-body">
                        <form class="form-horizontal" action="{{ route('admin.update_maintanance' ) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group row">
                                <label class="col-sm-2 control-label">{{ __('Maintanance Mode') }}<span
                                            class="text-danger">*</span></label>
                                <div class="col-sm-10">
                                    <input type="checkbox" {{ $commonsetting->is_maintainance_mode == '1' ? 'checked' : '' }} data-size="large" name="is_maintainance_mode" data-bootstrap-switch data-off-color="danger" data-on-color="primary" data-on-text="Active" data-label-text="<i class='fas fa-mouse'></i>"  data-off-text="Dactive">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-2 control-label">{{ __('Maintanance Image') }} <span class="text-danger">*</span></label>
                                <div class="col-sm-10">
                                    <img class="mw-400 mb-3 show-img img-demo" src="
                                    @if($commonsetting->maintainance_image)
                                    {{ asset('assets/front/img/'.$commonsetting->maintainance_image)}}
                                    @else
                                    {{ asset('assets/admin/img/img-demo.jpg') }}
                                    @endif"
                                    " alt="">
                                    <div class="custom-file">
                                        <label class="custom-file-label" for="maintainance_image">{{ __('Choose New Image') }}</label>
